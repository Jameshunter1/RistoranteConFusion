...
class CommentFormModal extends Component {
  ...
  validateComment() {
    if (this.state.comment.length < 10) {
      this.setState({ commentError: 'Comment must be at least 10 characters long' });
    } else {
...
const { rating, name, comment } = this.state;
const errors = { ratingError: null, nameError: null, commentError: null };
this.validateRating();
this.validateName();
this.validateComment();
if (!this.state.ratingError && !this.state.nameError && !this.state.commentError) {
this.props.postComment(this.props.dishId, rating, name, comment);
this.toggleModal();
} else {
          ...
          <ModalBody>
            <form onSubmit={this.handleSubmit}>
              <Row className='form-group'>
                ...
                <Col md={10}>
                  <input type='number' id='rating' name='rating' min='1' max='5' defaultValue='5' ref={(input) => (this.rating = input)} />
                </Col>
              </Row>
              <Row className='form-group'>
...
<Col md={10}>
<input type='text' id='name' name='name' placeholder='Your Name' value={this.state.name} onChange={this.handleNameChange} />
...
<Col md={10}>
<textarea id='comment' name='comment' rows='6' value={this.state.comment} onChange={this.handleCommentChange}></textarea>
{this.state.commentError && <p className='text-danger'>{this.state.commentError}</p>}
</Col>
</Row>
              <Row className='form-group'>
